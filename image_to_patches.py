...
from train_vit import prepare_dataloaders, set_seed
...
def image_to_patches(image, patch_size, image_grid = True):
    ...
    if image_grid:
        ...
        patches = rearrange(
            image,
            "b c (p_num_h p_height) (p_num_w p_width) -> b c p_num_h p_height p_num_w p_width",
            p_num_h= H // patch_h,
            p_num_w= W // patch_w,
            p_height=patch_h,
            p_width=patch_w,
        ...
        patches = rearrange(
            patches,
            "b c p_num_h p_height p_num_w p_width -> b (p_num_h p_num_w) c p_height p_width",
            p_num_h= H // patch_h,
            p_num_w= W // patch_w,
            p_height=patch_h,
            p_width=patch_w,
...
if __name__ == "__main__":
    ...
    img_grid = torchvision.utils.make_grid(
        example_images, nrow=batch_size, normalize=True, pad_value=0.9
    )
    ...
    for i in range(example_images.shape[0]):
        img_grid = torchvision.utils.make_grid(
            img_patches[i], nrow=img_patches.shape[1], normalize=True, pad_value=0.9
        )
        img_grid = img_grid.permute(1, 2, 0)
    ...
    print(
        f"shape of input patches BEFORE the linear projection layer (batch): {img_patches.shape}"
    )
    ...
    sns.heatmap(
        pos_embedding.squeeze(0), cmap=sns.color_palette("viridis", as_cmap=True)
    )
