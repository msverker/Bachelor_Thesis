...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        assert attention_matrix.size() == (batch_size * self.num_heads, seq_len, seq_len)
        assert out.size() == (batch_size, seq_len, embed_dim)

        return self.o_projection(out), attention_matrix
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        attention_out, attention_matrix = self.attention(x)
...
class ViT(nn.Module):
    ...
    def forward(self, img):
        ...
        if self.pos_enc == "fixed" and self.pool == "cls":
            positions = torch.cat(
                [torch.zeros(1, embed_dim).to(img.device), positions], dim=0
            )
        x = tokens + positions
        ...
        atten_matrix_list = []
        for transformer_block in self.transformer_list:
            x, atten_matrix = transformer_block(x)
            atten_matrix_list.append(atten_matrix)
        ...
        return self.classifier(x), atten_matrix_list
